...
int main(int argc, char **argv) {
    ...
    const int NUM_PARTICLES = atoi(argv[5]);  // Number of swarm particles
    const int MAX_ITERATIONS = atoi(argv[6]); // Maximum pso iterations 
   ...
   // Assign the appropriate function pointer based on user input
    ...
    printf("-------------------------------------------------------------------------\n");
    printf("PSO with adaptive w, c1 = 1.5, c2 = 1.5 initiated:\n");
    ...
    // The following loop means all the variables have the same lower and upper bounds
    ...
    // Measure the CPU time only for the pso function
    clock_t start, end;
    double cpu_time;
